...
# Đổi tên index 
desc.index = ["Số dòng", "Mean", "Std", "Min", "25%", "50% (Median)", "75%", "Max"]
desc.columns = ["Giá SP (R$)", "Phí Ship (R$)", "Tổng TT (R$)", "Review Score", "Thời gian giao (ngày)"]
...
col_a.metric("Tổng số dòng", f"{len(df):,}")
col_b.metric("Số cột", f"{df.shape[1]}")
missing_pct = (df.isnull().sum().sum() / df.size * 100)
col_c.metric("Tỉ lệ missing", f"{missing_pct:.2f}%")

with st.expander("📋 Xem chi tiết missing values theo cột"):
    missing = df.isnull().sum().reset_index()
    missing.columns = ["Cột", "Số dòng thiếu"]
    missing["Tỉ lệ (%)"] = (missing["Số dòng thiếu"] / len(df) * 100).round(2)
    missing = missing[missing["Số dòng thiếu"] > 0]
